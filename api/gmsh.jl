...
# This file defines the Gmsh Julia API (v5.0.0).
#
...
const GMSH_API_VERSION = "5.0.0"
const GMSH_API_VERSION_MAJOR = 5
const GMSH_API_VERSION_MINOR = 0
const GMSH_API_VERSION_PATCH = 0
const libdir = dirname(@__FILE__)
const libname = Sys.iswindows() ? "gmsh-5.0.dll" : "libgmsh"
import Libdl
...
Get the points `closestCoord` on the entity of dimension `dim` (1 or 2) and tag
`tag` to the points `coord`, by orthogonal projection. `coord` and
`closestCoord` are given as x, y, z coordinates, concatenated: [p1x, p1y, p1z,
p2x, ...]. `parametricCoord` returns the parametric coordinates t on the curve
(if `dim` == 1) or u and v coordinates concatenated on the surface (if `dim` =
...
space type `functionSpaceType`. `orientationSign` contains the correction
between each key and keyMaster to match basisFunction evaluation. If
`returnCoord` is set, the `coord` and `coordMaster` vectors contain the x, y, z
coordinates locating basis functions for sorting purposes.
...
are returned in the `steiner` vector, and the new tetrahedra in the `tetraOut`
vector.
