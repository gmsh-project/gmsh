...
Return the tag of the newly created overlap group, which can be passed as
`overlapIndex` to the query functions (tags are assigned sequentially from 0, so
the tag is also the position of the group).
